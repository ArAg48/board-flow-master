-- Phase 1: Implement password hashing and fix critical security issues

-- First, create a function to hash passwords using pgcrypto extension
...
-- Note: This will require users to reset their passwords since we can't reverse plain text
UPDATE public.profiles 
...
CREATE OR REPLACE FUNCTION public.log_security_event(event_type text, event_details jsonb DEFAULT '{}')
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
AS $$
BEGIN
  -- In a production environment, you'd want to log to a dedicated audit table
  -- For now, we'll use RAISE NOTICE to log to the database logs
  RAISE NOTICE 'Security Event: % - % - User: %', event_type, event_details, auth.uid();
END;
$$;