-- Fix get_ptl_order_progress to only count time from sessions that scanned boards
...
AS $function$
  ...
  LEFT JOIN (
    SELECT 
      ...
      SUM(ss.actual_duration_minutes) AS active_time
    FROM public.scan_sessions ss
    WHERE EXISTS (
      SELECT 1 FROM public.board_data bd 
      WHERE bd.ptl_order_id = ss.ptl_order_id 
