...
DROP FUNCTION IF EXISTS get_ptl_order_progress();

-- Create the new function that excludes sessions with no scanned boards
...
BEGIN
  ...
  ss_stats AS (
    ...
    WHERE total_scanned > 0  -- Only count sessions with scanned boards
    GROUP BY ptl_order_id
  )
  SELECT 
    po.id,
